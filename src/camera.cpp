...
Ray Camera::generate_ray(double x, double y) const {
  ...
  //x range [-tan(radians(hfov)*.5), tan(radians(hfov)*.5)]
  //y range [same. but with vfov]

  double xcam = -tan(radians(hFov)*.5) + (2*tan(radians(hFov)*.5))*x;
  double ycam = -tan(radians(vFov)*.5) + (2*tan(radians(vFov)*.5))*y;

  Vector3D camPos = Vector3D(xcam, ycam, -1);

  Vector3D worldPos = c2w * camPos;

  worldPos.normalize();

  Ray r = Ray(pos, worldPos);
