...
namespace CGL {
    ...
    Spectrum PathTracer::estimate_direct_lighting_hemisphere(const Ray& r, const Intersection& isect) {
        ...
        for (int i = 0; i < num_samples; i++)
        {
            ...
            Ray ray = Ray(hit_p + (EPS_D * wi_world), wi_world);

            bool hit = bvh->intersect(ray, &a);

            if (hit)
            {
                Spectrum irradiance = a.bsdf->get_emission() * isect.bsdf->f(wi_world, w_out) * wi.z;
                L_out += irradiance;
            }
        }
        return L_out/num_samples * 2 * PI;
    ...
    Spectrum PathTracer::estimate_direct_lighting_importance(const Ray& r, const Intersection& isect) {
        ...
        for (SceneLight* L: scene->lights)
        {
            ...
            if (L->is_delta_light())
            {
                Spectrum sample = L->sample_L(hit_p, &wi, &distToLight, &pdf);
                Vector3D w_in = w2o * wi;
                if (w_in.z < 0)
                {
                    continue;
                }
                Ray ray = Ray(hit_p + (EPS_D * wi), wi, distToLight);
                if (!bvh->intersect(ray, &a))
                {
                    //Calculate the BSDF val at w_out and w_in
                    Spectrum bsdf = isect.bsdf->f(w_out, w_in);
                    Spectrum irradiance = (sample * bsdf * w_in.z)/pdf;
                    L_out = L_out + irradiance;
                }
            } else {
                Spectrum temp;
                for (int i = 0; i < ns_area_light; i++)
                {
                    ...
                    if (!bvh->intersect(ray, &a))
                    {
                        ...
                        Spectrum bsdf = isect.bsdf->f(w_out, w_in);
                        Spectrum irradiance = (sample * bsdf * w_in.z)/pdf;
                        temp = temp + irradiance;
                    }
                }
                L_out = L_out + temp/(double) ns_area_light;
            }
        }
        return L_out;
    }
...
//  Russian Roulette
        if (coin_flip(.64) && r.depth > 1)
        {
            Ray ray = Ray(hit_p + (EPS_D * wi), wi, r.max_t, r.depth - 1);
            if (bvh->intersect(ray, &a))
            {
                Spectrum irradiance = (at_least_one_bounce_radiance(ray, a) * isect.bsdf->f(w_out, w_in) * w_in.z)/(pdf * .64);
                L_out = L_out + irradiance;
            }
        }
        return L_out;
...
//        Spectrum s;
//        if (num_samples == 1) {
//            Vector2D normedSample = Vector2D((x + .5f)/(double) sampleBuffer.w, (y+ .5f)/(double) sampleBuffer.h);
//            Ray r = camera -> generate_ray(normedSample.x, normedSample.y);
...
//            Vector2D normedSample = Vector2D((x + samplePoint.x)/(double) sampleBuffer.w, (y + samplePoint.y)/(double) sampleBuffer.h);
//            Ray r = camera -> generate_ray(normedSample.x, normedSample.y);
//            r.max_t = max_ray_depth;
//            s = s + est_radiance_global_illumination(r);
//        }
//        sampleCountBuffer[x + y * frameBuffer.w] = num_samples;
//        return s / (double) num_samples;
        ...
        if (ns_aa == 1) {
            ...
            if (samplesPerBatch)
            {
                ...
                while (j < ns_aa)
                {
                    ...
                    Ray r = camera -> generate_ray(normedSample.x, normedSample.y);
                    r.depth = max_ray_depth;
                    Spectrum est_glob_rad = est_radiance_global_illumination(r);
                    s = s + est_glob_rad;

                    double illuminance = est_glob_rad.illum();
                    s1 = s1 + illuminance;
                    s2 = s2 + (illuminance * illuminance);

                    if (j % samplesPerBatch == 0)
                    {
                        u = s1/j;
                        o2 = (s2 - ((s1 * s1)/j))/(j - 1);
